        ...
        if IWalkConfig.Combo then
              local target = GetTarget(1200, DAMAGE_MAGIC)
                if ValidTarget(target, 1200) then
                        ...
                        local QPred = GetPredictionForPlayer(GetMyHeroPos(),target,GetMoveSpeed(target),900,250,GetCastRange(myHero,_Q),200,false,false)
						...
						local EPred = GetPredictionForPlayer(GetMyHeroPos(),target,GetMoveSpeed(target),1200,250,GetCastRange(myHero,_E),120,false,false)
