        ...
        if IWalkConfig.Combo then
              local target = GetTarget(1000, DAMAGE_PHYSICAL)
                if ValidTarget(target, 1000) then
						...
						local WPred = GetPredictionForPlayer(GetMyHeroPos(),target,GetMoveSpeed(target),1025,250,GetCastRange(myHero,_W),73,true,true)
						...
						local EPred = GetPredictionForPlayer(GetMyHeroPos(),target,GetMoveSpeed(target),900,250,GetCastRange(myHero,_E),300,false,false)
