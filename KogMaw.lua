        ...
        if IWalkConfig.Combo then
              local target = GetTarget(1900, DAMAGE_PHYSICAL)
                if ValidTarget(target, 1900) then
					    ...
					    local QPred = GetPredictionForPlayer(GetMyHeroPos(),target,GetMoveSpeed(target),1650,300,1200,70,true,false)
			        ...
			        CastSpell(_W)
						...
						local RPred = GetPredictionForPlayer(GetMyHeroPos(),target,GetMoveSpeed(target),0,1250,1800,150,false,true)
