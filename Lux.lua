...
if IOW:Mode() == "Combo" then
        ...
        local RPred = GetPredictionForPlayer(GoS:myHeroPos(),target,GetMoveSpeed(target),0,1000,3500,110,false,false)
                      ...
                      for _, ally in pairs(GoS:GetAllyHeroes()) do
                        ...
                        if CanUseSpell(myHero, _W) == READY and WPred.HitChance == 1 and (GetCurrentHP(ally)/GetMaxHP(ally))<0.5 and GoS:ValidTarget(target, GetCastRange(myHero,_W)) and LuxMenu.Combo.AW:Value() then
...
if LuxMenu.Drawings.W:Value() then DrawCircle(GoS:myHeroPos().x, GoS:myHeroPos().y, GoS:myHeroPos().z,GetCastRange(myHero,_E),3,100,0xff00ff00) end
if LuxMenu.Drawings.E:Value() then DrawCircle(GoS:myHeroPos().x, GoS:myHeroPos().y, GoS:myHeroPos().z,GetCastRange(myHero,_W),3,100,0xff00ff00) end
